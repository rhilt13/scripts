...
## $3 = full path to the run_gaps profile (~/rhil_project/GT/gta_revise6/profile/gt_rev6)
## $4 = order file (~/rhil_project/GT/gta_revise6/profile/order)
######################################################
## Output:
...
######################################################
## To run:
## bash ~/rhil_project/GT/scripts/omcbpps_analysis.sh cazy_nongt2_pdb.p99_new.mma 1
## bash ~/rhil_project/GT/scripts/omcbpps_analysis.sh nr_nongt2_pdb.p90_new.mma 2 ~/rhil_project/GT/gta_revise3/profile/gt_rev2 ~/rhil_project/GT/gta_revise3/profile/order
## 
mkdir sets
cp $1 sets/
...
if [ $2 -eq 2 ]; then
	...
	for i in `ls *fa`; do run_gaps $3 $i -O &>>$i.hits; done
	...
	parse_omcbppsSets_hit_list.pl hits_details $4 > hits_details_parse 
...
cd ..
map_pdb_afteromcBPPS.sh $file_short
