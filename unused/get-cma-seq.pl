...
while(<IN>){
	...
	if ($size>1){
		...
		foreach $name(@b){
			...
			$name=~s/\s+$//g;
			if ($name=~/^I[0-9]+$/){
				push @{$hash{'internal'}{$a[0]}},$name;
			}else{
				push @{$hash{'tip'}{$a[0]}},$name;
		...
		# get seq from cma
		push @{$hash{'tip'}{$a[0]}},$a[0];
	}
}
# print Dumper(\%{$hash{'tip'}});
# =ccc
...
while(<IN2>){
	...
	if ($size==1){
		$level{$a[0]}=$j;
		$file=$directory."/".$name_hash{$a[0]}."_"."l".$j.".selID";
		open(OUT,">",$file);
		# print "================$a[0]\n";
		if (exists $hash{'tip'}{$a[0]}){
			foreach $id(@{$hash{'tip'}{$a[0]}}){
				print OUT "$id\n";
				# $printed{$id}=1;
			}
		}else{
		...
		close OUT;
		# foreach $id(sort keys %{$hash{'tip'}{$a[0]}}){
		# 	print "$a[0]\t$id\t@{$hash{'tip'}{$id}}\n";
		# }
	}else{
		push @check, $_;
	}
...
close IN2;
# if (exists $hash{'internal'})
$size = keys %level;

# print "$size out of $ct\n"; 
until ($size==$ct){
	# print $size; 
	$j++;
	print $j;
